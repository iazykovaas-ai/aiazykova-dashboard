...
SHEETS = {
    ...
    "deals_sdelki": {"spreadsheet_id": DEALS_SPREADSHEET_ID, "worksheet": "Сделки"},
}

# Данные кэшируются на 1 час (страховка). Плюс кэш сбрасывается при входе новой
# сессии (см. styles.apply) — «зашёл человек → данные свежие».
...
# Лист: D–M = месячные итоги (март…декабрь), с N (14) — по дням (даты в строке 1).
# (Лист переструктурировали ~07.2026: добавили колонки итогов июль…декабрь → дневные сдвинулись с H на N.)
MON_DAILY_START_COL = 14                       # колонка N — первая дневная
MON_MONTH_TOTAL_COLS = {m: m + 1 for m in range(3, 13)}   # март=D(4) … декабрь=M(13)
# Пустые будущие месяцы вернут NaN → в KPI покажется «—» (не выдумываем итог).
...
# ⚠️ Эти карты колонок — ТОЛЬКО ФОЛБЭК. Основной источник — sheets_loader.pl_month_maps(),
# который определяет колонки по маркерам «-MM-» в шапке (лист периодически «пакуют» — у закрытых
# месяцев убирают M2M-колонки, номера съезжают). Значения ниже — актуальный слепок на 11.09.2026.
# Факт 2026: Jan–Jul идут ПОДРЯД (17–23, без M2M), Aug–Dec — через M2M-колонку (25,27,29,31,33).
PL_FACT_2026_COLS = {1: 17, 2: 18, 3: 19, 4: 20, 5: 21, 6: 22, 7: 23,
                     8: 25, 9: 27, 10: 29, 11: 31, 12: 33}
# Колонки факта 2025: Jan=5 ... Dec=16 (подряд)
PL_FACT_2025_COLS = {m: 4 + m for m in range(1, 13)}
# Колонки бюджета 2026: Jan=45 (AS) ... Dec=56 (BD), подряд
PL_BUDGET_2026_COLS = {m: 44 + m for m in range(1, 13)}
...
PL_FULL_METRICS = [
    ("Оборот", [5], "money"),
    ("Валовая прибыль (маржа)", [35], "money"),
    ("Маржинальность", [45], "pct"),
    ("Переоценка", [54], "money"),
    ("Внутрибанковские конвертации", [55], "money"),
    ("OPEX (операционные расходы)", [57], "money"),
    ("  IT расходы", [58], "money"),
    ("  Маркетинг", [68], "money"),
    ("  Расходы на персонал", [76], "money"),
    ("  Общехоз. и админ. расходы", [102], "money"),
    ("  Консалтинг и аудит", [132], "money"),
    ("  Комплаенс", [138], "money"),
    ("Операционная прибыль", [145], "money"),
    ("Прочие доходы и расходы", [146], "money"),
    ("Финансовые доходы и расходы", [169], "money"),
    ("Хеджирование", [170, 171], "money"),
...
# Читаем ПОСДЕЛОЧНУЮ базу целиком → агрегируем по месяцам на лету (любой период — диапазон
...
DEALS_COL = {
    "month":       1,   # A — «янв.-26» … «авг.-26»
    "quarter":     2,   # B — «1 квартал 2026»
    "id":          3,   # C — ID_Deal
    "client":      4,   # D — Клиент
    "client_comb": 5,   # E — Клиент (объединенный) — для счёта уникальных
    "date":        6,   # F — Дата ДД/ММ/ГГ
    "product":     9,   # I — Импорт / Экспорт / Ликвидность
    "subtype":    10,   # J — Крипта / CASH / Торговые дома / Остальные / Крипта на вход/выход
    "cur_in":     12,   # L — валюта прихода (для подтипов обмена)
    "cur_out":    13,   # M — валюта ухода
    "channel":    31,   # AE — Собственные / Агентский / Банковский / Оптовые посредники / (ликвидность)
    "channel_sub":32,   # AF — Оптовые клиенты банков / Розничные с доступом / без доступа
    "sale":       33,   # AG — первичная / повторная
    "obmen":      38,   # AL — признак ОБМЕН: «да» / «нет»
}
# Денежные компоненты (1-индекс). Расходные хранятся с минусом → всегда прибавляем.
DEALS_MONEY = {
    "turnover":   16,   # P — Оборот
    "our_comm":   17,   # Q — Комиссия для клиента (бывш. «Наша комиссия»)
    "agent":      18,   # R — Комис. агента (≤0)
    "banks":      19,   # S — Комис. банкам (≤0)
    "fx":         20,   # T — Курс. разница
    "komma":      21,   # U — Комма за пп / списания
    "subagent":   22,   # V — Комис. субагента (≤0)
    "sub_fx":     23,   # W — Курс. разн. субагента
    "loans":      25,   # Y — Займы (≤0)
    "pl5470":     26,   # Z — PL 5470 (≤0)
    "fot":        27,   # AA — ФОТ процессинга (≤0)
    "interbank":  28,   # AB — Внутрибанковские конвертации
}
# Уровни дохода (фактические):
#   Валовая маржа  = our_comm + fx
#   Чистая маржа   = валовая + agent + loans          (сходится со столбцом AD листа)
#   Чистая прибыль = чистая + banks + komma + subagent + sub_fx + pl5470 + fot + interbank
DEALS_GROSS_ADD      = ["our_comm", "fx"]
DEALS_NET_MARGIN_ADD = ["agent", "loans"]
DEALS_NET_PROFIT_ADD = ["banks", "komma", "subagent", "sub_fx", "pl5470", "fot", "interbank"]

# Ликвидность (поставщики) — это снабжение, а не продажи. В витрине сделок смотрим ТОЛЬКО на
# КЛИЕНТСКИЕ сделки → ликвидность исключаем ВЕЗДЕ (в обоих разрезах). Маркеры: канал «(ликвидность)»
# ИЛИ продукт «Ликвидность» (наборы чуть различаются — убираем по любому из признаков).
DEALS_LIQUIDITY_CHANNEL = "(ликвидность)"
DEALS_LIQUIDITY_PRODUCT = "Ликвидность"
# Порядок верхнего уровня для разрезов. Прочие значения идут после известных.
DEALS_CH_ORDER = ["Собственные", "Агентский", "Оптовые посредники", "Банковский"]
DEALS_CH_BANK  = "Банковский"       # у этого канала показываем подкатегории (AF)
DEALS_PR_ORDER = ["Импорт", "Экспорт"]

# ОБМЕН — обменные клиентские сделки (флаг AL=«да», кроме ликвидности) выносятся в ОТДЕЛЬНУЮ
# группу в обоих разрезах — как в листах «…(новый порядок)»: Импорт/Экспорт и Собственные/
# Агентский/… считаются БЕЗ обмена, обмен = своя группа (без двойного счёта, сумма = ИТОГО).
# Подтипы обмена по валютам: приход (L) / уход (M).
DEALS_OBMEN_FLAG   = "да"
DEALS_OBMEN_GROUP  = "Обмен"
DEALS_RUB          = "RUB"
DEALS_OBMEN_SUB_ORDER = ["Рубль на вход", "Рубль на выход", "Без рубля"]
