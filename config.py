...
# Маркер строк ликвидности в столбце «Канал» — их в разрезе КАНАЛОВ исключаем (это снабжение,
# а не продажи). В разрезе ПРОДУКТОВ ликвидность остаётся отдельным продуктом (столбец I).
DEALS_LIQUIDITY_CHANNEL = "(ликвидность)"
# Порядок верхнего уровня для разрезов (как в сводах). Прочие значения идут после известных.
...
DEALS_PR_ORDER = ["Импорт", "Экспорт", "Ликвидность"]
