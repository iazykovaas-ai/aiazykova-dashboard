...
SHEETS = {
    ...
    "deals_channels": {"spreadsheet_id": DEALS_SPREADSHEET_ID, "worksheet": "Каналы (новый порядок)"},
    "deals_products": {"spreadsheet_id": DEALS_SPREADSHEET_ID, "worksheet": "Продукты (новый порядок)"},
}
...
# Листы «Каналы (новый порядок)» / «Продукты (новый порядок)». Внутри — по 3 блока-периода
# (1 кв / 2 кв / июль-2026). Начало блока ищем по строке, где кол. A = «Период», кол. B = имя.
# Столбцы B:E — витрина (имя, клиенты, сделки, средний чек). Столбцы O:AC — СЫРЫЕ суммы из
# «Сделок», НЕ зависят от переключателя режима D2 → из них честно считаем фактические уровни.
DEALS_NAME_COL   = 2    # B — название канала/продукта
DEALS_CLIENTS_COL = 3   # C — уникальных клиентов, шт
DEALS_DEALS_COL  = 4    # D — сделок, шт
DEALS_PERIOD_MARK = "Период"    # метка в колонке A строки-заголовка блока

# Сырые суммы (1-индекс колонок). money-компоненты аддитивны между блоками.
DEALS_RAW_COLS = {
    "turnover":   15,   # O — Оборот
    "our_comm":   16,   # P — Наша комиссия
    "client_exp": 17,   # Q — *Расход клиента (наша комиссия + гипотетическая скрытая надбавка)
    "fx":         18,   # R — Курс. разница
    "agent":      19,   # S — Комис. агента (≤0)
    "loans":      21,   # U — Займы (≤0)
    "banks":      22,   # V — Комис. банкам (≤0)
    "komma":      23,   # W — Комма за пп / списания
    "subagent":   24,   # X — Комис. субагента (≤0)
    "sub_fx":     25,   # Y — Курс. разн. субагента
    "pl5470":     26,   # Z — PL 5470 (≤0)
    "fot":        27,   # AA — ФОТ процессинга (≤0)
    "interbank":  28,   # AB — Внутрибанковские конвертации
}
# Уровни дохода (все — из сырых сумм, фактические, «режим 1»):
#   Валовая маржа = our_comm + fx
#   Чистая маржа  = валовая + agent + loans
...
# Классификация строк ВНУТРИ блока по названию (кол. B). level: 0 — верхний уровень
# (образуют полное разбиение оборота), 1 — вложенная детализация, "memo" — справочно
# (первичные/повторные), None — пропустить (пустые «в том числе», второй ИТОГО).
DEALS_TOTAL_NAME = "ИТОГО"
# Каналы: 4 канала верхнего уровня; банковский разложен на 3 подстроки.
DEALS_CH_LEVEL0 = ["Собственные клиенты", "Агентский канал",
                   "Оптовые посредники (не банки)", "Банковский канал итого"]
DEALS_CH_LEVEL1 = ["Оптовые клиенты банков", "Розничные клиенты с доступом",
                   "Розничные клиенты без доступа"]
DEALS_CH_MEMO   = ["первичные продажи", "повторные продажи"]
# Продукты: 3 продукта верхнего уровня; каждый разложен на подтипы.
DEALS_PR_LEVEL0 = ["Импорт (клиентский) всего", "Экспорт (клиентский)",
