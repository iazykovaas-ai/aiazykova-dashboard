...
combo_seg = st.selectbox("Сегмент для графика по дням", ["Все сегменты"] + MON_LINES,
                         format_func=lambda x: x if x == "Все сегменты" else MON_LINE_LABELS_RU.get(x, x),
                         key="mon_combo_seg")
if combo_seg == "Все сегменты":
    ...
    _seg_note = f"сегмент: {MON_LINE_LABELS_RU.get(combo_seg, combo_seg)} · без Other / Agent / Gold"
