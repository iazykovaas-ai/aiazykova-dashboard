...
df_sorted = df.sort_values("Оборот, М ₽", ascending=False).reset_index(drop=True)
fig = go.Figure()
for i, (_, row) in enumerate(df_sorted.iterrows()):
    ...
    fig.add_trace(cuboid_mesh(
        x0=i - 0.35, x1=i + 0.35,
        y0=-0.35, y1=0.35,
        z0=0, z1=row["Оборот, М ₽"],
        color=color,
        name=f"{row['Тип']}: {row['Оборот, М ₽']:.1f} М ₽ · {row['Кол-во']} клиентов",
    ))
fig.update_layout(
    scene=dict(
...
with right:
    ...
    df_avg = df.assign(**{
        "Средний": (df["Оборот, М ₽"] / df["Кол-во"]).round(2),
    ...
    fig = go.Figure(go.Bar(
        ...
        textposition="outside",
        textfont=dict(color=PALETTE["ink"], size=12),
        hovertemplate="<b>%{y}</b><br>%{x:.2f} М ₽ / клиент<extra></extra>",
    ))
    style_plotly_2d(fig, height=400)
    fig.update_layout(xaxis=dict(showgrid=True, showticklabels=False),
                      yaxis=dict(showgrid=False))
...
display = df.sort_values("Оборот, М ₽", ascending=False).copy()
